function ydata = sym_sne_p(P, labels, no_dims, perplexity)
...
% University of California, San Diego
    ...
    for iter=1:max_iter
        ...
        num = exp(-(1 + bsxfun(@plus, sum_ydata, bsxfun(@plus, sum_ydata', -2 * (ydata * ydata'))))); % Normal distribution
...
%         sigma_low        
%         x = [min(A):0.01:max(A)];
%         norm = normpdf(x,Mu_low,sigma_low);
%         figure(2);
%         plot(x,norm)
        ...
        if ~rem(iter, 10) && ~isempty(labels)
            ...
            elseif no_dims == 2
                scatter(ydata(:,1), ydata(:,2), 28, cmap(labels+1,:), 'filled');
                title('Plot for t-SNE');
                xlabel('Distance');
                ylabel('Probability');
                drawnow
            else
                scatter3(ydata(:,1), ydata(:,2), ydata(:,3), 40, cmap(labels+1,:), 'filled');
            end
        end
%     s = num2str(perplexity);
%     s
%     fname = 'C:\Users\siffi\Desktop\Hw07_results\symmetric-sne';
%     saveas(gcf,fullfile(fname, s), 'jpeg');
    end
