function ydata = sym_sne(X, labels, no_dims, initial_dims, perplexity)
%TSNE Performs symmetric-SNE on dataset X
%
%   mappedX = tsne(X, labels, no_dims, initial_dims, perplexity)
%   mappedX = tsne(X, labels, initial_solution, perplexity)
%
% The function performs symmetric-SNE on the NxD dataset X to reduce its 
...
% data are not used by symmetric-SNE itself, however, they are used to color
...
%     plot(x,norm)
    ...
    % Run t-SNE
    if initial_solution
        ydata = sym_sne_p(P, labels, ydata, perplexity);
    else
        ydata = sym_sne_p(P, labels, no_dims, perplexity);
    end
